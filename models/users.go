...
var UserRels = struct {
	UIDAccessKeys string
}{
	UIDAccessKeys: "UIDAccessKeys",
}

// userR is where relationships are stored.
type userR struct {
	UIDAccessKeys AccessKeySlice `boil:"UIDAccessKeys" json:"UIDAccessKeys" toml:"UIDAccessKeys" yaml:"UIDAccessKeys"`
}
...
func (r *userR) GetUIDAccessKeys() AccessKeySlice {
	if r == nil {
		return nil
	}
	return r.UIDAccessKeys
}
...
// UIDAccessKeys retrieves all the access_key's AccessKeys with an executor via uid column.
func (o *User) UIDAccessKeys(mods ...qm.QueryMod) accessKeyQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"access_keys\".\"uid\"=?", o.UID),
	)

	return AccessKeys(queryMods...)
}

// LoadUIDAccessKeys allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadUIDAccessKeys(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		qm.From(`access_keys`),
		qm.WhereIn(`access_keys.uid in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if err != nil {
		return errors.Wrap(err, "failed to eager load access_keys")
	}

	var resultSlice []*AccessKey
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice access_keys")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on access_keys")
	}
	if err = results.Err(); err != nil {
		return errors.Wrap(err, "error occurred during iteration of eager loaded relations for access_keys")
	}
	...
	if singular {
		object.R.UIDAccessKeys = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
				foreign.R = &accessKeyR{}
			}
			foreign.R.UIDUser = object
		}
		return nil
	}
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.UID == foreign.UID {
				local.R.UIDAccessKeys = append(local.R.UIDAccessKeys, foreign)
				if foreign.R == nil {
					foreign.R = &accessKeyR{}
				}
				foreign.R.UIDUser = local
				break
			}
		}
	}
...
// AddUIDAccessKeys adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.UIDAccessKeys.
// Sets related.R.UIDUser appropriately.
func (o *User) AddUIDAccessKeys(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*AccessKey) error {
	var err error
	for _, rel := range related {
		if insert {
			...
			values := []interface{}{o.UID, rel.Key}
	...
	if o.R == nil {
		o.R = &userR{
			UIDAccessKeys: related,
		}
	} else {
		o.R.UIDAccessKeys = append(o.R.UIDAccessKeys, related...)
	}
...
func FindUser(ctx context.Context, exec boil.ContextExecutor, uID int64, selectCols ...string) (*User, error) {
	userObj := &User{}
	...
	query := fmt.Sprintf(
		"select %s from \"users\" where \"uid\"=$1", sel,
	)
...
func (o *User) Reload(ctx context.Context, exec boil.ContextExecutor) error {
	ret, err := FindUser(ctx, exec, o.UID)
	if err != nil {
		return err
	}
...
func UserExists(ctx context.Context, exec boil.ContextExecutor, uID int64) (bool, error) {
	var exists bool
...
func (o *User) Exists(ctx context.Context, exec boil.ContextExecutor) (bool, error) {
	return UserExists(ctx, exec, o.UID)
}
