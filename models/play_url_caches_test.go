...
var (
	playURLCachDBTypes = map[string]string{`ID`: `integer`, `EpisodeID`: `bigint`, `IsVip`: `boolean`, `Area`: `smallint`, `DeviceType`: `smallint`, `Data`: `json`, `CreatedAt`: `timestamp without time zone`, `UpdatedAt`: `timestamp without time zone`}
	_                  = bytes.MinRead
)
