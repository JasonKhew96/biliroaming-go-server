...
func testUsersFind(t *testing.T) {
	...
	userFound, err := FindUser(ctx, tx, o.UID)
	if err != nil {
		t.Error(err)
	}
...
func testUserToManyUIDAccessKeys(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = randomize.Struct(seed, &b, accessKeyDBTypes, false, accessKeyColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, accessKeyDBTypes, false, accessKeyColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	...
	check, err := a.UIDAccessKeys().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := UserSlice{&a}
	if err = a.L.LoadUIDAccessKeys(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.UIDAccessKeys); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.UIDAccessKeys = nil
	if err = a.L.LoadUIDAccessKeys(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.UIDAccessKeys); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testUserToManyAddOpUIDAccessKeys(t *testing.T) {
	var err error
	...
	var a User
	var b, c, d, e AccessKey

	seed := randomize.NewSeed()
	if err = randomize.Struct(seed, &a, userDBTypes, false, strmangle.SetComplement(userPrimaryKeyColumns, userColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
	foreigners := []*AccessKey{&b, &c, &d, &e}
	for _, x := range foreigners {
		if err = randomize.Struct(seed, x, accessKeyDBTypes, false, strmangle.SetComplement(accessKeyPrimaryKeyColumns, accessKeyColumnsWithoutDefault)...); err != nil {
			t.Fatal(err)
		}
	}
	...
	foreignersSplitByInsertion := [][]*AccessKey{
		{&b, &c},
		{&d, &e},
	}

	for i, x := range foreignersSplitByInsertion {
		err = a.AddUIDAccessKeys(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.UID != first.UID {
			t.Error("foreign key was wrong value", a.UID, first.UID)
		}
		if a.UID != second.UID {
			t.Error("foreign key was wrong value", a.UID, second.UID)
		}

		if first.R.UIDUser != &a {
			t.Error("relationship was not added properly to the foreign slice")
		}
		if second.R.UIDUser != &a {
			t.Error("relationship was not added properly to the foreign slice")
		}

		if a.R.UIDAccessKeys[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.UIDAccessKeys[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.UIDAccessKeys().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
...
var (
	userDBTypes = map[string]string{`UID`: `bigint`, `Name`: `character varying`, `VipDueDate`: `timestamp without time zone`, `CreatedAt`: `timestamp without time zone`, `UpdatedAt`: `timestamp without time zone`}
	_           = bytes.MinRead
)
